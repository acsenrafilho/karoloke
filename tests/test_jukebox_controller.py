...
BACKGROUND_FOLDER = os.path.join(pathlib.Path(__file__).parents[1], 'karoloke','backgrounds')

def test_get_background_img_raise_error_for_empty_image_list(tmpdir):
    with pytest.raises(FileNotFoundError, match="No background images found in the 'background' directory."):
